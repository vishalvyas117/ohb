...
public interface HotelRepository extends CrudRepository<Hotel, Integer>, JpaSpecificationExecutor<Hotel> {
	// @Query(nativeQuery = true,value = "select ho from Hotel as ho where
	// ho.name in ?1")
	@QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value = "true") })
	List<Hotel> findHotelsByNameContaining(String name);

	/*@Query(nativeQuery = true, value = "select * from Hotel as ho where ho.hotel_name = ?1 and ho.hotel_address=?2")
	@QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value = "true") })
	Hotel checkExcistingHotel(String hotelName, String address);*/

	/*
	 * @QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value =
	 * "true") }) Hotel findOneByHotel_id(Integer hotel_id);
	 */

	/*
	 * @Query(value =
	 * "SELECT ho,ho.hotelid from Hotel as ho left join ho.name as ho2 group by ho.city having count(ho.hotelid) <= 3"
	 * ) List<Hotel> findtop3HotelsforeachCity();
	 */

	// @Query(nativeQuery = true, value = "select ho from Hotel ho where ho.city
	// = :city_id")
	@QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value = "true") })
	 ...
	 * "select * from Hotel as ho join room as ro on ro.hotel_id=ho.hotel_id")
	 * 
	 * @QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value =
	 * "true") }) List<Hotel> findHotelswithRooms();
	 */

	// @Query(nativeQuery = true, value = "select ho from Hotel ho where
	// ho.address in %?1%")
